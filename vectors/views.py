    ...
    def get(self, request):
        ...
        if query:
            ...
            products = Product.objects.order_by(
                L2Distance('title_embedding', query_embedding))[:10]
        else:
            products = Product.objects.none()
        serializer = ProductSerializer(products, many=True)
        return Response(serializer.data)
...
def image_search(request):
    if request.method == 'POST':
        ...
        if text:
            ...
            products = Product.objects.order_by(L2Distance(
                'image_embedding', text_embedding[0]))[:10]
        elif image:
            image = Image.open(image).convert('RGB')
            embedding = image_model.encode([image], convert_to_tensor=True)
            products = Product.objects.order_by(
                L2Distance('image_embedding', embedding[0]))[:10]
            urls = [product.image.url for product in products]
            print(urls)
    else:
        products = Product.objects.none()
